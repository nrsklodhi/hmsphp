<?php
	ob_start();
	session_start();
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <title>Home</title>
    <meta charset="utf-8">
    <link rel="stylesheet" href="css/reset.css" type="text/css" media="screen">
    <link rel="stylesheet" href="css/style.css" type="text/css" media="screen">
    <link rel="stylesheet" href="css/grid.css" type="text/css" media="screen"> 
    <script src="js/jquery-1.6.2.min.js" type="text/javascript"></script>
    <script src="js/jquery.galleriffic.js" type="text/javascript"></script>
    <script src="js/jquery.opacityrollover.js" type="text/javascript"></script>      
	<!--[if lt IE 7]>
        <div style=' clear: both; text-align:center; position: relative;'>
            <a href="http://www.microsoft.com/windows/internet-explorer/default.aspx?ocid=ie6_countdown_bannercode"><img src="http://storage.ie6countdown.com/assets/100/images/banners/warning_bar_0000_us.jpg" border="0"  alt="" /></a>
        </div>
	<![endif]-->
    <!--[if lt IE 9]>
   		<script type="text/javascript" src="js/html5.js"></script>
        <link rel="stylesheet" href="css/ie.css" type="text/css" media="screen">
	<![endif]-->
</head>
<body id="page1">
	<!--==============================header=================================-->
<?php include('header.php');?>

<!-- content -->
  <section id="content">
        <div class="bg-top">
        	<div class="bg-top-2">
                <div class="bg">
                    <div class="bg-top-shadow">
                        <div class="main">
                            <div class="gallery p3">
                            	<div class="wrapper indent-bot">
                                    <div id="gallery" class="content">
                                       <div class="wrapper">
                                           <div class="slideshow-container">
                                                <div id="slideshow" class="slideshow"></div>
                                            </div>
                                        </div>
                                    </div>
                                    <div id="thumbs" class="navigation">
                                        <ul class="thumbs noscript">
                                            <li>
                                                <a class="thumb" href="images/gallery-img1.jpg" title=""> <img src="images/thumb-1.jpg" alt="" /><span></span> </a>
                                            </li> 
                                            <li>
                                                <a class="thumb" href="images/gallery-img2.jpg" title=""> <img src="images/thumb-2.jpg" alt="" /> <span></span></a>
                                            </li> 
                                            <li>
                                                <a class="thumb" href="images/gallery-img3.jpg" title=""> <img src="images/thumb-3.jpg" alt="" /> <span></span></a>
                                            </li> 
                                            <li>
                                                <a class="thumb" href="images/gallery-img4.jpg" title=""> <img src="images/thumb-4.jpg" alt="" /> <span></span></a>
                                            </li> 
                                            <li>
                                                <a class="thumb" href="images/gallery-img5.jpg" title=""> <img src="images/thumb-5.jpg" alt="" /> <span></span></a>
                                            </li> 
                                            <li>
                                                <a class="thumb" href="images/gallery-img6.jpg" title=""> <img src="images/thumb-6.jpg" alt="" /> <span></span></a>
                                            </li>           
                                        </ul>
                                    </div>
                                </div>
                                 </div>
                            <div class="container_12">
                            	<div class="wrapper">
                                	<article class="grid_12">
                                    	<h3 class="color-1">Events Services Specialities</h3>
                                        <div class="wrapper">
                                        	<article class="grid_6 alpha">
                                              
                                              <article class="grid_6 omega">
                                                <figure class="img-indent frame"><img src="images/_DSC2720.jpg" alt="" width="200" height="130" /></figure>
                                                 <ul class="list-1">
                                                             <li><a href="#">Wooden and Acrylic Dance Floors</a></li>
                                                   
                                           <li><a href="#">Make up and Beauty</a></li>                   
                                                             
                                                               <li><a href="#">Catering Services</a></li>
                                                             
                                                        </ul>
                                            
                                                
                                                <div class="clear"></div>
                                            </article>
                                              <div class="clear"></div>
                                            </article>
                                            <article class="grid_6 omega">
                                                <figure class="img-indent frame"><img  src="images/mainimg.jpg"alt="" width="200" height="130" /></figure>
                                                 <ul class="list-1">
                                                 <li><a href="#">Videography and Photography</a></li>
                                                             <li><a href="#">Event Lighting & Effects</a></li>
                                                             <li><a href="#">Flower Decoration Setup</a></li>
                                                            
                                                             
                                                        </ul>
                                            
                                                
                                                <div class="clear"></div>
                                            </article>
                                        </div>
                                    </article>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>	
        </div>
        <div class="bg-bot">
        	<div class="main">
            	<div class="container_12">
   	  <div class="wrapper">
                    	<article class="grid_4">
                        	<h3 class="prev-indent-bot">About Us</h3>
                            <p class="prev-indent-bot">This website <a target="_blank" href="home.php ">NRsK's Event Organizers</a> &nbsp; provides you the best available services of event management for our clients.</p>
                            <p class="prev-indent-bot">At present our company is being know among the best wedding planners in Pakistan. If you are seeking for wedding organizers who can provide you excellent services then <a target="_blank" href="home.php ">NRsK's Event Organizers</a> is just the right name.</p>
                            <p class="prev-indent-bot">We are here to facilitate you and give you the most reliable sevices making your events ever remembered and well-organized at affordable rates.</p>
                    	</article>
                        <article class="grid_4">
                        	<h3 class="prev-indent-bot">Testimonials</h3>
                            <div class="quote">
                            	<p class="prev-indent-bot">It was my one of best and ever remebered experience to organize my daughters wedding by NRsK's Event Organizers. Our planner (NRsK's) was specialized, clued-up, and uncompromising, which I believe is what you want in a wedding planner. We did not have to worry about anything because Al Waseem Events management did it for us. Accept our thanks for providing us a perfect event. We are grateful to you.</p>
                                <h5>NEETIRAJ SINGH(NRsK)</h5>
                                The Last year's Client
                            </div>
                        </article>
                      <article class="grid_4">
                       	  <h3 class="prev-indent-bot">Login to Organize Events!</h3>
                          
                          <form name="u_login" action="welcome.php" method="post">
                            <table align="center" cellspacing="10">
                            <tr>
                            <td ><h5>UserName</h5></td>
                            <td><input name="u_name" type="text" size="30" maxlength="30"></td></tr>
                            <tr><td><h5>Password</h5></td>
                            <td><input name="pass" type="password" size="30" maxlength="30"></td></tr>
                            
                          <td colspan="2" align="center"><input name="u_login" type="submit"></td>
                          <tr><td colspan="2"><br></td></tr>
                          </table>
                          </form>
                          <?php
	if(isset($_REQUEST['u_login']))
	{
		 $u_name=$_REQUEST['u_name'];
	   $u_pass=$_REQUEST['pass'];
	    include "hoteldb_connection.php";
		$sql="Select * From user_table Where User_name='$u_name' AND U_Password='$u_pass'";
		$rs = mysql_query($sql) or die(mysql_error());
	if(mysql_num_rows($rs)>0)
	{
		
	while($row = mysql_fetch_array($rs))
	{
		
		$_SESSION['s_uname']=$row['User_name'];
		$_SESSION['s_uid']= $row['User_id'];
        //header('refresh:1, url=welcomeadmin.php');
	
			}
	
	}
	else
	{
		echo"<font color=#663300 size=+4>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Invalid login</font>";
	}
	
	}
	?>
                        
                          
               	   <h5> Not a member??<h5>
               	    <h3 class="prev-indent-bot"><a href="">Register your self!</a></h3>
                   
                         
                        <p class="prev-indent-bot">&nbsp;</p>
                      
                  </article>
                  </div>
                </div>
            </div>
        </div>
</section>
    
	<!--==============================footer=================================-->
    <?php include('footer.php');?>
<script type="text/javascript">
			$(window).load(function() {
			// We only want these styles applied when javascript is enabled
			$('div.navigation').css({'width' : '320px', 'float' : 'right'});
			$('div.content').css('display', 'block');
	
			// Initially set opacity on thumbs and add
			// additional styling for hover effect on thumbs
			var onMouseOutOpacity = 0.5;
			$('#thumbs ul.thumbs li span').opacityrollover({
				mouseOutOpacity:   onMouseOutOpacity,
				mouseOverOpacity:  0.0,
				fadeSpeed:         'fast',
				exemptionSelector: '.selected'
			});
			
			// Initialize Advanced Galleriffic Gallery
			var gallery = $('#thumbs').galleriffic({
				delay:                     7000,
				numThumbs:                 12,
				preloadAhead:              6,
				enableTopPager:            false,
				enableBottomPager:         false,
				imageContainerSel:         '#slideshow',
				controlsContainerSel:      '',
				captionContainerSel:       '',
				loadingContainerSel:       '',
				renderSSControls:          true,
				renderNavControls:         true,
				playLinkText:              'Play Slideshow',
				pauseLinkText:             'Pause Slideshow',
				prevLinkText:              'Prev',
				nextLinkText:              'Next',
				nextPageLinkText:          'Next',
				prevPageLinkText:          'Prev',
				enableHistory:             true,
				autoStart:                 7000,
				syncTransitions:           true,
				defaultTransitionDuration: 900,
				onSlideChange:             function(prevIndex, nextIndex) {
					// 'this' refers to the gallery, which is an extension of $('#thumbs')
					this.find('ul.thumbs li span')
						.css({opacity:0.5})
				},
				onPageTransitionOut:       function(callback) {
					this.find('ul.thumbs li span').css({display:'block'});
				},
				onPageTransitionIn:        function() {
					this.find('ul.thumbs li span').css({display:'none'});
				}
			});
		});
	</script>
</body>
</html>
